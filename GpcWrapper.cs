namespace GpcWrapper
{
	...
	public class GpcWrapper
	{
        ...
        {		
            [DllImport( "gpc.dll" )]
            public static extern void gpc_polygon_to_tristrip( [In]     ref gpc_polygon  polygon,
            ...
            [DllImport( "gpc.dll" )]
            public static extern void gpc_tristrip_clip( [In]     GpcOperation     set_operation,
                                                          [In]     ref gpc_polygon  subject_polygon,
                                                          [In]     ref gpc_polygon  clip_polygon,
            ...
            [DllImport( "gpc.dll" )]
            public static extern void gpc_read_polygon( [In] IntPtr fp, [In] int read_hole_flags, [In,Out] ref gpc_polygon polygon );

            [DllImport( "gpc.dll" )]
            public static extern void gpc_write_polygon( [In] IntPtr fp, [In] int write_hole_flags, [In] ref gpc_polygon polygon );

            [DllImport( "msvcr71.dll", SetLastError = true, CharSet = CharSet.Unicode )]
            public static extern IntPtr fopen( [In] string filename, [In] string mode );
            
            [DllImport( "msvcr71.dll" )]
            public static extern void fclose( [In] IntPtr fp );
		...
		public static Tristrip PolygonToTristrip( Polygon polygon )
		{
			...
			NativeMethods.gpc_polygon_to_tristrip( ref gpc_pol, ref gpc_strip );
			Tristrip tristrip = GpcWrapper.Gpc_strip_ToTristrip( gpc_strip );
			
			GpcWrapper.Free_gpc_polygon( gpc_pol );
			GpcWrapper.NativeMethods.gpc_free_tristrip( ref gpc_strip );
		...
		public static Tristrip ClipToTristrip( GpcOperation operation, Polygon subject_polygon, Polygon clip_polygon )
		{
			...
			NativeMethods.gpc_tristrip_clip( operation, ref gpc_subject_polygon, ref gpc_clip_polygon, ref gpc_strip );
			Tristrip tristrip = GpcWrapper.Gpc_strip_ToTristrip( gpc_strip );
		...
		public static Polygon Clip( GpcOperation operation, Polygon subject_polygon, Polygon clip_polygon )
		{
			...
			NativeMethods.gpc_polygon_clip( operation, ref gpc_subject_polygon, ref gpc_clip_polygon, ref gpc_polygon );
			Polygon polygon = GpcWrapper.Gpc_polygon_ToPolygon( gpc_polygon );

			GpcWrapper.Free_gpc_polygon( gpc_subject_polygon );
			GpcWrapper.Free_gpc_polygon( gpc_clip_polygon );
			GpcWrapper.NativeMethods.gpc_free_polygon( ref gpc_polygon );
		...
		public static void SavePolygon( string filename, bool writeHoleFlags, Polygon polygon )
		{
			...
			IntPtr fp = NativeMethods.fopen( filename, "wb" );
			NativeMethods.gpc_write_polygon( fp, writeHoleFlags?((int)1):((int)0), ref gpc_polygon );
			NativeMethods.fclose( fp );
		...
		public static Polygon ReadPolygon( string filename, bool readHoleFlags )
		{
			...
			IntPtr fp = NativeMethods.fopen( filename, "rb" );
			NativeMethods.gpc_read_polygon( fp, readHoleFlags?((int)1):((int)0), ref gpc_polygon );
		...
		private static gpc_polygon PolygonTo_gpc_polygon( Polygon polygon )
		{
			...
			for ( int i=0 ; i<polygon.NofContours ; i++ ) {
                gpc_vertex_list gpc_vtx_list = new gpc_vertex_list()
                {
                    ...
                    vertex = Marshal.AllocCoTaskMem(polygon.Contour[i].NofVertices * Marshal.SizeOf(new gpc_vertex()))
                };
		...
		private static Tristrip Gpc_strip_ToTristrip( gpc_tristrip gpc_strip )
		{
            ...
            {
                NofStrips = gpc_strip.num_strips
            };
